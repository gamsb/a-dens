...
class AuctionTest_second_bidder_wins_auction_with_12(AuctionTest): 

    def test_auction_info_created(self): 
        self.assertEqual('test_name', b2s(self.auction.call_getter('m_name')))
        self.assertEqual(self.nic_name_timespan, self.auction.call_getter('m_nic_name_timespan'))
        self.assertEqual(self.now + self.nic_name_timespan, self.auction.call_getter('m_nic_name_starts_at'))
        self.assertEqual(self.now + self.nic_name_timespan - 40, self.auction.call_getter('m_bid_phase_ends_at'))
        self.assertEqual(self.now + self.nic_name_timespan, self.auction.call_getter('m_ends_at'))
        print("Auction info: ", self.auction.call_getter('getAuctionInfo'))

    def test_second_bidder_wins_auction_with_12(self):        
        seed = 54
        (b2_pk, b2_pu) = ts4.make_keypair()

        bidder2 = ts4.BaseContract('TestAuctionBidder', dict(bid_key = b2_pu, seed = seed), pubkey=b2_pu, private_key=b2_pk)
